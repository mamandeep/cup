$(document).ready(function () {
    ...
    var
        gradeTable = $('#paper-table'),
        gradeBody = gradeTable.find('tbody'),
        gradeTemplate = _.template($('#paper-template').remove().text()),
    ...
    gradeTable
        .on('click', 'a.add', function(e) {
            ...
            $(gradeTemplate({key: numberRows++}))
                ...
                .fadeIn('fast');
            if(numberRows > 1) {
                var attri1 = 'input[name$="data[Researchpaper][0][applicant_id]"]';
                var attri2 = 'input[name$="data[Researchpaper][' + (numberRows - 1) +  '][applicant_id]"]';
            ...
            $("#modified_papers").val('true');
        })
        .on('click', 'a.remove', function(e) {
                e.preventDefault();
            idElem = $("[name='Researchpaper.0.id']");
            userIdElem = $("[name='Researchpaper.0.applicant_id']");
